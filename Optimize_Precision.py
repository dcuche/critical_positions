...
CONFIG = {
    ...
    'output_file': 'Successors_Sample_optimized_PRECISION.xlsx',
    'embedding_model': 'text-embedding-3-large',
    'embedding_size': 3072,

    # --- Optimization Parameters ---
    'optimization_target_class': 'Critical',
    'precision_target': 0.90, # ### NEW: Explicitly define the precision goal ###
    'class_labels': ['Critical', 'Important', 'Moderate'],
    'ground_truth_column': 'Desired_Outcome',
    'optimization_method': 'differential_evolution',
    # ### MODIFIED: Increased search parameters for a more thorough optimization ###
    'n_iterations': 500,  # Drastically increase iterations
    'popsize': 30,        # Increase population size (popsize * num_params = # evaluations per generation)
    'tol': 0.001,         # A tighter tolerance for convergence
    'min_weight_value': 0.01, # Allow weights to be smaller if needed
...
# --- Helper Functions (unchanged, but added more robust error handling) ---
...
def calculate_criticality(dataf, params_config, full_config):
    ...
    # Handle edge case where quantiles might be identical after optimization step
    if critical_q <= important_q:
        df['Category'] = 'Moderate' # Assign a default, non-critical category
        return df
...
class Optimizer:
    def __init__(self, config, processed_data):
        ...
        self.precision_target = config['precision_target']
        ...
        print(f"Optimizer will primarily target a Precision of >{self.precision_target:.2f} for '{self.target_class}'.")
        print(f"Secondary objective: Maximize Recall for '{self.target_class}' without dropping precision.")
    ...
    # ### MODIFIED: The core of the solution - a new objective function ###
    def objective_function(self, flat_params_vector):
        params_config = self._unpack_parameters(flat_params_vector)

        # Constraint: critical_q must be > important_q. Return worst score if violated.
        ...
        # This is our new tiered loss function. We want to MINIMIZE this value.
        # It's designed to first achieve the precision target, then maximize recall.
        precision_gap = self.precision_target - p_score
        
        if precision_gap > 0:
            # PENALTY_MODE: We are below the precision target.
            # The main penalty is how far we are from the target precision.
            # We add a smaller penalty for low recall to guide the search.
            loss = (precision_gap * 10) + (1 - r_score) 
        else:
            # TARGET_MET_MODE: We have reached our precision goal.
            # Now, the only goal is to maximize recall.
            # Minimizing (1 - recall) is the same as maximizing recall.
            loss = (1 - r_score)

        return loss
    
    # ### NEW: Callback function to monitor progress ###
    def callback(self, xk, convergence):
        ...
        if loss < self.best_score:
            ...
            self.best_params = xk
            print(f"Iter: {self.iteration_count:03d} | New Best Loss: {loss:.4f} -> [Precision: {p_score:.3f}, Recall: {r_score:.3f}]")
    ...
    def run(self):
        ...
        result = differential_evolution(
            ...
            callback=self.callback, # Use the callback
            disp=False, # We have our own display now
            polish=True
        )
        end_time = time.time()
        ...
        return self.best_params # Return the best params found during the entire run
