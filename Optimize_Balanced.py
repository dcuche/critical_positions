...
CONFIG = {
    ...
    'output_file': 'Successors_Sample_optimized_PERFECTION.xlsx',
    ...
    # ### HEAVILY MODIFIED: Force a deep, exhaustive search ###
    'n_iterations': 1500,  # Massively increase iterations
    'popsize': 40,         # Increase population size for more exploration
    'tol': 0.0001,         # Very tight tolerance; don't stop until improvement is tiny
    'recombination': 0.7,  # Hyperparameter to encourage broader search (0 to 1)
    'min_weight_value': 0.0, # Allow features to have zero weight

    # ### NEW: Make PCA components configurable to add more features ###
    # We are giving the model more granular information from the text fields.
    'text_features': {
    ...
    'quantile_bounds': {
        'critical_q': (0.60, 0.99), # Widen the search space for thresholds
...
class Optimizer:
    def __init__(self, config, processed_data):
        ...
        print(f"Optimizer goal: Maximize (Precision + Recall) for '{self.target_class}'. Striving for a perfect match.")
        self.numeric_cols = self.config['numeric_features']['columns']
        # ### MODIFIED: Correctly build the list of final components based on the new PCA config ###
    ...
    def _unpack_parameters(self, flat_params_vector):
        ...
        w_idx = 1
        # ### MODIFIED: Correctly unpack weights for variable PCA components ###
    ...
    # ### HEAVILY MODIFIED: The new "perfectionist" objective function ###
    def objective_function(self, flat_params_vector):
        ...
        if params_config['quantiles']['critical_q'] <= params_config['quantiles']['important_q']:
            return 2.0 # Worst possible score is 2 (1-0 + 1-0)
        ...
        # The new loss function. We want to minimize this.
        # Loss = (1 - Precision) + (1 - Recall)
        # A perfect score (P=1, R=1) results in a loss of 0.
        # The worst score (P=0, R=0) results in a loss of 2.
        loss = (1 - p_score) + (1 - r_score)
        return loss
    ...
    def callback(self, xk, convergence):
        ...
        if loss < self.best_score:
            ...
            self.best_params = xk
            p_score = 1 - (loss - (1 - recall_score(self.y_true, calculate_criticality(self.optimization_data, self._unpack_parameters(xk), self.config)['Category'], labels=[self.target_class], average='micro', zero_division=0.0)))
            r_score = recall_score(self.y_true, calculate_criticality(self.optimization_data, self._unpack_parameters(xk), self.config)['Category'], labels=[self.target_class], average='micro', zero_division=0.0)
            print(f"Iter: {self.iteration_count:04d} | New Best Loss: {loss:.4f} -> [Precision: {p_score:.3f}, Recall: {r_score:.3f}]")
...
def main():
    ...
    # ### MODIFIED: Use the new configurable PCA components ###
    ...
    print(f"Best Loss (2 - P - R) Found: {optimizer.best_score:.4f}")
    
    # Final display and evaluation logic... (similar to before)
